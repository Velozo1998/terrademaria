...
const emptyForm = { nome: '', categoria: 'terço', custo: '', preco_venda: '', estoque_atual: '', estoque_minimo: '5' }
...
export default function Produtos() {
  ...
  function abrirModal(produto = null) {
    if (produto) {
      ...
      setForm({ nome: produto.nome, categoria: produto.categoria, custo: produto.custo, preco_venda: produto.preco_venda, estoque_atual: produto.estoque_atual, estoque_minimo: produto.estoque_minimo })
    } else {
      setEditando(null)
      setForm(emptyForm)
    }
  ...
  async function salvar() {
    ...
    setSalvando(true)
    const payload = { nome: form.nome.trim(), categoria: form.categoria, custo: Number(form.custo) || 0, preco_venda: Number(form.preco_venda), estoque_atual: Number(form.estoque_atual) || 0, estoque_minimo: Number(form.estoque_minimo) || 5 }
    let error
    if (editando) {
    ...
    else { showMsg(editando ? 'Produto atualizado!' : 'Produto cadastrado!', 'success'); fecharModal(); loadProdutos() }
    setSalvando(false)
  }
      ...
      {aba === 'ativos' && (
        <>
          ...
          {loading ? <p style={{ color: 'var(--texto-leve)', padding: 24 }}>Carregando...</p> : filtrados.length === 0 ? (
            <div className="card empty-state"><div className="icon">📿</div><h3>Nenhum produto encontrado</h3><p>Cadastre seu primeiro produto clicando em "+ Novo Produto"</p></div>
          ) : (
            <div className="table-wrapper">
              <table>
                <thead>
                  <tr><th>Produto</th><th>Categoria</th><th>Custo</th><th>Preço Venda</th><th>Margem</th><th>Estoque</th><th>Status</th><th style={{ textAlign: 'right' }}>Ações</th></tr>
                </thead>
                <tbody>
                  {filtrados.map(p => {
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td><span style={{ fontWeight: 700, color: Number(p.preco_venda) > Number(p.custo) ? 'var(--success)' : 'var(--danger)', fontSize: 13 }}>{margem(p.custo, p.preco_venda)}</span></td>
        ...
        <div className="modal-overlay" onClick={fecharModal}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group"><label className="form-label">Nome do Produto *</label><input className="form-input" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Ex: Terço de Nossa Senhora" /></div>
              <div className="form-group"><label className="form-label">Categoria</label><select className="form-input" value={form.categoria} onChange={e => setForm(f => ({ ...f, categoria: e.target.value }))}>{CATEGORIAS.map(c => <option key={c} value={c} style={{ textTransform: 'capitalize' }}>{c}</option>)}</select></div>
              <div className="grid-2">
                <div className="form-group"><label className="form-label">Custo (R$)</label><input className="form-input" type="number" step="0.01" min="0" value={form.custo} onChange={e => setForm(f => ({ ...f, custo: e.target.value }))} placeholder="0,00" /></div>
              ...
              {form.custo && form.preco_venda && (
                <div style={{ background: 'var(--bege)', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>
                  💡 Margem: <strong style={{ color: 'var(--success)' }}>{margem(form.custo, form.preco_venda)}</strong> · Lucro por unidade: <strong>{fmt(Number(form.preco_venda) - Number(form.custo))}</strong>
                </div>
              )}
              <div className="grid-2">
